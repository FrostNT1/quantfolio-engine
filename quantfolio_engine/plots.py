...
def plot_backtest_results(
    ...
    if 'sharpe_ratio' in performance_df.columns:
        ax2.plot(performance_df.index, performance_df['sharpe_ratio'], 
    ...
    if 'max_drawdown' in performance_df.columns:
        ax3.fill_between(performance_df.index, performance_df['max_drawdown'], 0, 
                        alpha=0.3, color='red')
        ax3.plot(performance_df.index, performance_df['max_drawdown'], 
    ...
    if 'volatility' in performance_df.columns:
        ax4.plot(performance_df.index, performance_df['volatility'], 
    ...
    if 'turnover' in performance_df.columns:
        ax5.bar(performance_df.index, performance_df['turnover'], 
    ...
    if 'transaction_cost' in performance_df.columns:
        ...
        ax6.plot(cumulative_costs.index, cumulative_costs.values, 
...
def plot_performance_comparison(
    ...
    ax1 = axes[0, 0]
    if 'total_return' in performance_df.columns and 'benchmark_total_return' in performance_df.columns:
        x = range(len(performance_df))
        ax1.bar([i-0.2 for i in x], performance_df['total_return'], 
                width=0.4, label='Portfolio', alpha=0.7, color='blue')
        ax1.bar([i+0.2 for i in x], performance_df['benchmark_total_return'], 
                width=0.4, label='Benchmark', alpha=0.7, color='red')
    ...
    ax2 = axes[0, 1]
    if 'sharpe_ratio' in performance_df.columns and 'benchmark_sharpe_ratio' in performance_df.columns:
        ax2.scatter(performance_df['benchmark_sharpe_ratio'], performance_df['sharpe_ratio'], 
    ...
    ax3 = axes[1, 0]
    if 'volatility' in performance_df.columns and 'total_return' in performance_df.columns:
    ...
    ax4 = axes[1, 1]
    if 'total_return' in performance_df.columns and 'benchmark_total_return' in performance_df.columns:
        outperformance = performance_df['total_return'] > performance_df['benchmark_total_return']
        hit_rate = outperformance.mean()
        ax4.pie([hit_rate, 1-hit_rate], labels=['Outperform', 'Underperform'], 
...
def plot_aggregate_metrics(
    ...
    ax1 = axes[0, 0]
    key_metrics = ['avg_total_return', 'avg_sharpe_ratio', 'avg_sortino_ratio', 'hit_ratio']
    metric_names = ['Total Return', 'Sharpe Ratio', 'Sortino Ratio', 'Hit Ratio']
    values = [metrics.get(metric, 0) for metric in key_metrics]
    
    bars = ax1.bar(metric_names, values, color=['blue', 'green', 'orange', 'red'], alpha=0.7)
    ...
    ax4.bar(turnover_names, turnover_values, color=['darkgreen', 'lightgreen'], alpha=0.7)
...
def plot_return_distribution(
    ...
    if 'total_return' in performance_df.columns:
        sns.histplot(performance_df['total_return'], bins=bins, color='blue', label='Portfolio', kde=True, stat='density', alpha=0.6, ax=ax)
    if 'benchmark_total_return' in performance_df.columns:
        sns.histplot(performance_df['benchmark_total_return'], bins=bins, color='red', label='Benchmark', kde=True, stat='density', alpha=0.4, ax=ax)
